...
export class sdwFilter extends HTMLElement {
    ...
    async load () {
        ...
        this.refs["filter"].change = () => {
            let refList = this.refs["filter-list"]
            let value = this.refs["filter"].value
    ...
    setVisualization () {
        let refList = this.refs["filter-list"]
        let filter = app.refSelected.getStyle("filter")

        this.filterShowForm("initial")

        if (filter == "" || filter == "initial") {
            this.refs["filter"].setValueWithParams("initial", false, false)
